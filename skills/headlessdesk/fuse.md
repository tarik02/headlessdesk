...
Read JSON/text files directly from `$HD_MOUNT`.

For PNG files, only view the FUSE path directly with image viewing. Never run
`file`, `stat`, `cp`, `cat`, `identify`, `xxd`, or similar shell/file probes on
`screenshot.png` or `crop/*.png`, and never copy them to `/tmp` or another
staging path.
...
```text
$HD_MOUNT/crop/100,100,400,300.png
```
...
Pass the path itself to image viewing:
